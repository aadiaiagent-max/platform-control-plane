...
A zero-runtime-dependency TypeScript library for multi-tenant feature flags, kill switches, percentage rollouts, and an append-only audit log.

Designed as a **platform control plane**: tenants configure flags independently; evaluation is deterministic; every mutation is auditable. Small surface area — readable in one sitting.
